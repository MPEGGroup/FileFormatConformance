...
def csv_to_spec(csv_file, feature_type):
    ...
    for row in csv_reader:
        ...
        entry = {"fourcc": code, "description": description, "containers": []}
...
def update_entry_from_gpac(specification, feature_type, gpac_entry):
    ...
    gpac_only_entries = SPECS[specification]["features"][feature_type][
        "gpac_only_entries"
    ]
        ...
        if gpac_entry.has_attr("grouping_type"):
            entry = next(
                (
                    item
                    for item in entries
                    if item["fourcc"] == gpac_entry["grouping_type"]
                ),
                None,
            )
    else:
        entry = next(
            (item for item in entries if item["fourcc"] == gpac_entry["type"]), None
        )
    ...
    if entry is not None:
        ...
        if gpac_entry["type"] == "00000000":
            return
        print(
            f'Not found: {gpac_entry["type"]} is missing in MP4RA {specification}:{feature_type}'
        )
...
def extract_spec_features_gpac():
    ...
    process = subprocess.Popen(
        ["MP4Box", "-boxes"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    ...
    for box in soup.boxes:
        ...
        if box.name == "trackreferencetypebox":
            ...
            # Everyting else is treated as a box, however we might want to split to other types,
...
def extract_spec_features():
    parser = argparse.ArgumentParser(
        description="Use MP4RA and GPAC MP4Box to create a set of standard features"
    )
    parser.add_argument(
        "-o",
        "--out",
        help="Output directory where files will be written to",
        default="./out",
    )
    ...
    for entry in data["entries"]:
        ...
        if params["type"] is not None:
            if "type" not in entry:
                ...
                print(
                    f'type mismatch in "{entry["fourcc"]}"! {entry["type"]} vs {params["type"]}'
                )
...
def check_spec_features():
    ...
    parser.add_argument(
        "-i",
        "--input",
        help="Input json file with spec features. Or folder with json files",
    )
    args = parser.parse_args()
